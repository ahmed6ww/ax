...
#
#   DIST=latest ./scripts/npm-publish.sh ./npm/launcher
#
# Two things this exists to prevent, both of which bit the 0.1.0 release:
#
#   1. `npm publish npm/axur` (the launcher's old directory name) is not a
#      path. npm reads `owner/repo` as a GitHub shorthand and tried to clone
#      github.com/npm/axur. Only a path starting with ./ or ending in / is
#      unambiguous, so this script normalises whatever it is given.
#
